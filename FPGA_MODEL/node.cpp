...
void node_network(par_t H[NHITS], par_t H_out[NHITS], par_t inbound[NHITS], par_t outbound[NHITS]){
...
// void node_runner(i_data_t edge_index[NEDGES*2], data_t e[NEDGES], par_t inbound[NEDGES], par_t outbound[NEDGES], par_t H[NHITS], par_t H_out[NHITS]){
void node_runner(par_t inbound[NHITS], par_t outbound[NHITS], par_t H[NHITS], par_t H_out[NHITS]){
  // #pragma HLS PIPELINE
  ...
  for(int i = 0; i < NHITS; i++){
    ...
    //   if(src == i_data_t(-1) || (src != i_data_t(i) && dst != i_data_t(i))){
    //     break;
    //   }
    ...
    par_t sum_in = inbound[i];
    par_t sum_out = outbound[i];
